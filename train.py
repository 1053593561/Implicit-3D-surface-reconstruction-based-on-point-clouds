...
def main(config):
    ...
    optimizer= torch.optim.Adam(net.parameters(), lr=config["training_lr_start"])
    ...
    checkpoint = torch.load(os.path.join('./weights_final/ShapeNet_3k/ShapeNet_3k_Normals_FKAConv_SA_InterpAttentionKHeadsNet_None', "checkpoint.pth"),map_location=device)
    ...
    while True:
        ...
        # if error/len(t) < error_best:
        #     error_best = error/len(t)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--config', '-c', type=str, default="configs/config_shapenet.yaml")
    parser.update_file_arg_names(["config_default", "config"])

    config = parser.parse(use_unknown=True)
    config["normals"]=True
    config["random_noise"]=0
    config["experiment_name"]='3K'
