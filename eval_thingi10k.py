...
parser.add_argument("--gendir", type=str, help="Path to generated data", default='./pred_result/Thingi10k/ECA/gen_Thingi10kTest/meshes/04_pts/')
parser.add_argument("--gtdir", type=str, help="Path to ground truth data", default='./new_datasets/thingi10k_scans_original/03_meshes/')
parser.add_argument("--meshdir", type=str, default="meshes")
parser.add_argument("--filter", type=str, default=None)
parser.add_argument("--npoints", type=int, default=100000)
parser.add_argument("--Fthreshold", type=float, default=0.01)
...
def process_meshes(source_filename, F_threshold):
    ...
    pred_filename = args.gendir+pred_filename
    if os.path.splitext(pred_filename)[1] != ".ply":
        pred_filename = pred_filename+".ply"

    logging.info(f"{os.getppid()} - {os.getpid()} -  {fname} - {F_threshold}")
    ...
    print(f"{os.getppid()} - {os.getpid()} - {fname} - done procesing")
    return out_dict
...
if __name__ == "__main__":
    ...
    eval_dataset = ABCEval(args.gtdir,
                split="test",
    ...
    out_file_class = os.path.join('./eval_results/eval_thingi10k_%s.csv' % args.gendir.split('/')[3])
