...
def _chamfer_distance_single_file(file_in, file_ref, samples_per_model, num_processes=1):
    ...
    ref_new_dist, corr_new_ids = kdtree_new_mesh_samples.query(ref_mesh_samples, 1)
    new_ref_dist, corr_ref_ids = kdtree_ref_mesh_samples.query(new_mesh_samples, 1)
...
def mesh_comparison(new_meshes_dir_abs, ref_meshes_dir_abs,
    ...
    res = np.array([list(item[2:]) for item in results], dtype=float)
    res = res.mean(axis=0)
